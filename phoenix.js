...
const applyLayout = (layoutScheme) => {
  ...
  Window.all()
    ...
    .forEach(window => {
      ...
      const applicableRule = Object.keys(layout).find(rule => new RegExp(rule).test(title));
...
const moveToRelativeSpace = direction => () => {
  ...
  const nextSpace = direction >= 0 ? currentSpace.next() : currentSpace.previous();
...
Key.on("q", ["shift", "alt"], () => moveToScreen(tallMonitor)(Window.focused()));
Key.on("w", ["shift", "alt"], () => moveToScreen(middleMonitor)(Window.focused()));
